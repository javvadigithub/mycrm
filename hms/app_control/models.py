...
class InventoryGroup(models.Model):
    ...
    def save(self, *args,  **kwargs):
        ...
        add_user_activity(self.create_by, action=action)
...
class Inventory(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new:
            ...
            add_user_activity(create_by, action=action)
...
class Store(models.Model):
    created_by = models.ForeignKey(
        CustomUser, null=True, related_name="store",
        on_delete=models.SET_NULL
    )
    ...
    def save(self, *args,  **kwargs):
        action = f"added new store - '{self.name}'"
        if self.pk is not None:
            action = f"updated str[] from  - '{self.old_name}' to '{self.name}'"
        super().save(*args, **kwargs)
        add_user_activity(self.create_by, action=action)

    def delete(self, *args, **kwargs):
        created_by = self.create_by
        action = f"deleted store - '{self.name}'"
        super().delete(*args,**kwargs)
        add_user_activity(created_by, action=action)
...
class Invoice(models.Model):
    ...
    store = models.ForeignKey(
        Store, related_name="sale_store", null=True, on_delete=models.SET_NULL)
    created_at = models.DateTimeField(auto_now_add=True)
