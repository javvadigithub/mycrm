...
from .serializers import (
    ...
    Store, StoreSerializer, Invoice, InvoiceSerializer, InventoryWithSumSerializer,
    StoreWithAmountSerializer, InvoiceItem
)

from rest_framework.response import Response
from hms.custom_methods import IsAuthenticatedCustom
from hms.utils import CustomPagination, get_query
...
class InvoiceView(ModelViewSet):
    queryset = Invoice.objects.select_related(
        "created_by", "store").prefetch_related("invoice_items")
    ...
    def get_queryset(self):
        ...
        if keyword:
            search_fields = (
                "created_by__fullname", "created_by__email", "store__name"
            )
            query = get_query(keyword, search_fields)
            results = results.filter(query)
...
class SummaryView(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        total_store = StoreView.queryset.count()
...
class SaleByStoreView(ModelViewSet):
    http_method_names = ('get',)
    permission_classes = (IsAuthenticatedCustom,)
    queryset = InventoryView.queryset
    ...
    def list(self, request, *args, **kwargs):
        ...
        if monthly:
            stores = query.annotate(month=TruncMonth('created_at')).values(
                'month', 'name').annotate(amount_total=Sum(
                    F("sale_store__invoice_items__quantity") * 
                    F("sale_store__invoice_items__amount")
                ))

        else:
            stores = query.annotate(amount_total=Sum(
                    F("sale_store__invoice_items__quantity") * 
                    F("sale_store__invoice_items__amount")
                )).order_by("-amount_total")

        response_data = StoreWithAmountSerializer(stores, many=True).data
        return Response(response_data)
